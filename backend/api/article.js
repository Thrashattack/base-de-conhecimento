...
module.exports = app => {
    ...
    const get = async (req,res) => {
        ...
        const resultado = await app.db('articles').count('id').first
    ...
    const getByCategory = async (req, res) => {
        ...
        app.db({a: 'articles', u: 'users'}) // from articles e users
        .select('a.id', 'a.name', 'a.description', 'a.imageUrl', {author: 'u.name'}) 
        .limit(limit).offset(page * limit - limit)
        .whereRaw('?? = ??', ['u.id', 'a.userId']) // user id == articleUserId
        .whereIn('categoryId', ids) // article belongs to the categories map
        .orderBy('a.id', 'desc')
